...
def populate_stats():
    ...
    health_metrics_response = requests.get(
        ...
        params={'start_timestamp': last_updated,
                'end_timestamp': current_datetime_formatted})
    ...
    if activity_logs_response.status_code == 200 and health_metrics_response.status_code == 200:
        ...
        average_duration = sum(log['duration'] for log in activity_logs) / num_activity_logs if num_activity_logs > 0 else 0
        average_heart_rate = sum(metric['value'] for metric in health_metrics if metric['metricType'] == 'Heart Rate') / num_health_metrics if num_health_metrics > 0 else 0
